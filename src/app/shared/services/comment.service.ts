...
export class CommentService {
  ...
  getComments(params: CommentsParamsType): Observable<CommentsCountType | DefaultResponseType> {
    return this.http.get<CommentsCountType | DefaultResponseType>(environment.api + 'comments', {
      params: {
        ...params,
        limit: 10
      }
  ...
  updateActionComment(commentId: string, params: {action: string} ): Observable<DefaultResponseType> {
    return this.http.post<DefaultResponseType>(environment.api + 'comments/' + commentId + '/apply-action',params)
  }
